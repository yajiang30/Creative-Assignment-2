...
// shows the window that asks for difficulty
...
function shuffle(array) {
  ...
  while (currentIndex !== 0) {
    ...
    currentIndex -= 1;
    swapCurIndexWithRandomIndex();
  }
  return array;

  function swapCurIndexWithRandomIndex() {
...
// function initializes and starts the game
function startGame() {
  let cards = [];
  cards = initializeDeckBasedOnDifficulty(cards);
  const shuffledDeck = shuffle(cards);
  for (let i = 0; i < shuffledDeck.length; i++) {
    const { liTag, divTag } = createDomElements(i);
    addAnimations(liTag);
...
function initializeDeckBasedOnDifficulty(cards) {
  if (gameMode === 'easy') {
...
function resetEverything() {
  ...
  if (gameMode === 'easy') {
    deck.classList.remove('easy');
  } else if (gameMode === 'medium') {
    deck.classList.remove('medium');
  } else {
    deck.classList.remove('hard');
  }
  startModal.style.display = 'block';
}
...
// calls this when the two cards opened match
function match() {
...
// calls this when the two cards opened dont match
function noMatch() {
...
// function manipulates the DOM and add the stats before displaying
function AddStats() {
